...
5. Inter-service communication usually has three forms:
   - HTTP (Refit): see `docs/wiki/en/service-http-call.md`
   - gRPC: see `docs/wiki/en/service-grpc-call.md`
   - Events (CAP): see `docs/wiki/en/service-event-call.md`
...
Used for "loading configuration." It supports local files and Consul KV (see `docs/wiki/en/config-center.md` for details):
...
Used for "service registration and discovery." It supports `Direct/Consul/CoreDns` (for configuration, see `docs/wiki/en/appsettings.md`; for usage, see `docs/wiki/en/registry-center.md`):
...
- **Queries/validation**: use HTTP or gRPC to get results synchronously and keep the call chain as short as possible  
  - HTTP: `docs/wiki/en/service-http-call.md`
  - gRPC: `docs/wiki/en/service-grpc-call.md`
- **Cross-service write collaboration/eventual consistency**: prefer events, which make decoupling easier and can also reduce call chains  
  - CAP: `docs/wiki/en/service-event-call.md`

## 3.4 Authentication and Authorization

In "service-to-service call" scenarios, ADNC provides a more engineering-oriented approach: it supports both Basic and Bearer passthrough, reducing repeated Token-handling work in business code (see `docs/wiki/en/claims-based-authentication.md`).

## 3.5 Caching, Distributed Locks, and Bloom Filters

Caching-related capabilities, including handling common issues such as cache avalanche, cache breakdown, and cache penetration, are collected in one document and are suitable for direct implementation (see `docs/wiki/en/cache-redis-distributedlock-bloomfilter.md`).

## 3.6 Data Access and Transactions (EF Core + UnitOfWork)

Repositories, unit of work, local transactions/distributed transactions, native SQL, and related topics all have corresponding demos and documentation (you can start with the `docs/wiki/en/efcore-*.md` series).
...
- Tracing: SkyAPM (SkyWalking .NET Agent). For how to enable it, see `docs/wiki/en/skyapm-tracing.md`.
...
- Logging and auditing: the project has unified logging configuration and output methods (you can start with the Logging node in `docs/wiki/en/appsettings.md`).

---

## 4. How to Run It Locally (The Minimum Things You Need to Change)

If you only want to "run it and take a look" first, follow the order in `docs/wiki/en/quickstart.md`. The core items are just these three:
...
1. `docs/wiki/en/quickstart.md`: run it first
2. `docs/wiki/en/appsettings.md`: understand the key configuration nodes
3. `docs/wiki/en/config-center.md`: configuration center (if you need multiple environments or unified configuration delivery)
4. `docs/wiki/en/registry-center.md`: registry center (if you need real service discovery and load balancing)
5. `docs/wiki/en/service-http-call.md` / `docs/wiki/en/service-grpc-call.md`: how to write synchronous calls, carry authentication, and configure addresses
6. `docs/wiki/en/service-event-call.md`: how event-driven code handles idempotency, transactions, and retries
7. `docs/wiki/en/feature-dev-guide.md` + `docs/wiki/en/api-dev-guide.md` + `docs/wiki/en/service-dev-guide.md`: follow the conventions to add a CRUD feature
8. Read `docs/wiki/en/skyapm-tracing.md` when you need to troubleshoot performance or call chains
9. Read `docs/wiki/en/quickly-docker-deploy.md` when you need to deploy to a server or container
