...
- KV 初始化脚本：`deploy/staging/adnc-consul/consul-init.sh`
- KV 初始数据：`deploy/staging/adnc-consul/kv.json`
...
- Compose 中默认把 Consul UI 暴露到宿主机端口 `8590`（`deploy/staging/adnc-consul/docker-compose.yml`）。
